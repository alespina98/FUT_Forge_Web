;(()=>{
  ...
  const ffTrack=(event,properties)=>{try{const id=ffInstallId||ffSessionId;if(!id)return;const body=JSON.stringify({events:[{event,timestamp:Date.now(),client_type:"bookmarklet",client_version:"2.10.5-browser.12",install_id:id,session_id:ffSessionId||id,properties:properties||{}}]});const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),2500);fetch("https://futforgeofficial.com/api/analytics/events",{method:"POST",mode:"cors",credentials:"omit",keepalive:true,headers:{"content-type":"application/json"},body,signal:controller.signal}).catch(()=>{}).finally(()=>clearTimeout(timer));}catch(_){}};
