;(()=>{
  ...
  if(globalThis.__FUTFORGE_BOOKMARKLET_LOADED__||globalThis.FutForgeDispatcher)return alert("FUT Forge is already running on this page.");
  ...
  if(!loaderUrl)return fail("The remote loader URL is unavailable.");
  ...
  }catch(error){globalThis.__FUTFORGE_LOADER_ACTIVE__=false;const c=String(error?.message||error);fail(c==="ea_runtime_not_ready"?"EA Web App is not ready. Log in fully, then try again.":c==="integrity_mismatch"?"The downloaded release failed its integrity check.":"Could not load the browser release.",error);}})();
})();