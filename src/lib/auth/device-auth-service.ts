...
export async function approveBrowserAuthorization(userCode:string,clerkUserId:string){
  ...
  const result=await getDeviceAuthStore().decide(normalizeUserCode(userCode),clerkUserId,profile.id,"APPROVE");if(!result||result.clientType!=="browser")throw new Error("NOT_PENDING");return result;
}
export async function pollBrowserAuthorization(deviceCode:string){
  const store=getDeviceAuthStore(),polled=await store.poll(await sha256(deviceCode));if(polled.kind!=="approved")return polled;
  const approved=polled.authorization!;if(approved.clientType!=="browser"||!approved.applicationUserId||!await getIdentityRepository().getUserByApplicationId(approved.applicationUserId)){await store.rejectApproved(approved.id);return{kind:"denied" as const}}
  const consumed=await store.consume(approved.id);if(!consumed?.applicationUserId)return{kind:"consumed" as const};return{kind:"tokens" as const,tokens:await issue(consumed.applicationUserId,"browser")};
}
...
async function refreshSession(refreshToken:string,expectedClientType?:FutForgeClientType){const store=getDeviceAuthStore(),next=randomToken(32),rotated=await store.rotateRefresh(await sha256(refreshToken),await sha256(next),new Date(),expectedClientType);if(rotated.kind!=="rotated"){await store.event("REFRESH_FAILURE",expectedClientType??null);return rotated}const accessToken=await signFutForgeAccessToken({applicationUserId:rotated.applicationUserId,clientType:rotated.clientType,scope:rotated.scope});await store.event("REFRESH_SUCCESS",rotated.clientType);return{kind:"tokens" as const,tokens:{access_token:accessToken,token_type:"Bearer",expires_in:futForgeTokenMetadata.accessTokenLifetimeSeconds,refresh_token:next,scope:rotated.scope.join(" ")}}
}
