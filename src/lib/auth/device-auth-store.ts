...
export class DeviceAuthStore{
  ...
  async rotateRefresh(oldHash:string,newHash:string,now=new Date()){
    const transaction=await this.client.transaction("write");
    try{const found=await transaction.execute({sql:"SELECT * FROM device_refresh_tokens WHERE token_hash=?",args:[oldHash]});const row=found.rows[0];if(!row){await transaction.rollback();return{kind:"invalid" as const}}const familyId=String(row.family_id);
  ...
  async revokeFamily(tokenHash:string,now=new Date()){const found=await this.client.execute({sql:"SELECT family_id FROM device_refresh_tokens WHERE token_hash=?",args:[tokenHash]});if(!found.rows[0])return false;await this.client.execute({sql:"UPDATE device_refresh_tokens SET status='REVOKED',revoked_at=? WHERE family_id=? AND status='ACTIVE'",args:[iso(now),String(found.rows[0].family_id)]});return true}
