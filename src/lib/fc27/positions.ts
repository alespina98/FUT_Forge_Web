...
import { isFc27ArtifactError, readArtifact } from "./static-data";
...
export const fetchPositionCounts = unstable_cache(async (): Promise<PositionCount[]> => {
  try{const rows=await readArtifact<PositionCount[]>("positions/counts.json");return FC27_POSITIONS.map(position=>({position,count:rows.find(row=>row.position===position)?.count??0}))}catch(error){if(!isFc27ArtifactError(error))throw error;console.warn(`[FC27 DATA] static artifact unavailable: positions (${error.artifact})`)}
  return Promise.all(FC27_POSITIONS.map(async (position) => {
    ...
    const response = await boundedFc27SupabaseFetch(`position-${position}`,`${url}/rest/v1/fc27_players?${params}`, {
