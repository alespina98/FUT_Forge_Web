...
export function Fc27StatFinderView({result,options,initial}:{result:StatFinderResult;options:FilterOptions;initial:Record<string,string>}){
  const {t}=useI18n();const c=t.fc27StatFinderPage;const outfieldLabels=["PAC","SHO","PAS","DRI","DEF","PHY"];const goalkeeperLabels=["DIV","HAN","KIC","REF","SPD","POS"];
  return <div className="fc27-stat-page hero-grid relative px-4 pb-24 pt-36 sm:px-6 sm:pt-40"><div className="hero-noise"/><div className="hero-orb hero-orb-primary"/><div className="relative mx-auto max-w-6xl">
    ...
    <section className="mt-8" aria-labelledby="stat-results"><div className="fc27-stat-results-head"><div><h2 id="stat-results">{c.results}</h2><p aria-live="polite">{c.resultCount.replace("{count}",result.total.toLocaleString())}</p></div><small>{result.elapsedMs} ms</small></div>
      {result.players.length===0?<div className="fc27-ranking-empty"><p>{c.noPlayers}</p><span>{c.relaxFilters}</span><Link href="/fc27/stat-finder">{c.resetFilters}</Link></div>:<ol className="fc27-stat-results">{result.players.map((player,index)=>{const href=`/fc27/players/${playerUrlSlug(player.ea_player_id,player.slug)}`;const stats=[player.pace,player.shooting,player.passing,player.dribbling,player.defending,player.physicality];const labels=player.position_short_label==="GK"?goalkeeperLabels:outfieldLabels;return <li className="fc27-stat-row" key={player.ea_player_id}>
