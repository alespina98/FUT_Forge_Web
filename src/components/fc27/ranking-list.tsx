...
export function Fc27RankingList({ players, stat, labels }: { players: RankingPlayer[]; stat: RankingStat; labels: Labels }) {
  return <ol className="fc27-ranking-list">{players.map((player,index)=>{const href=`/fc27/players/${playerUrlSlug(player.ea_player_id,player.slug)}`;return <li key={player.ea_player_id} className={`fc27-ranking-row rank-${index+1}`}>
