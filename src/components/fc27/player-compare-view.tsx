...
export function Fc27CompareView({ aId, bId, playerA, playerB, baseMetaA, baseMetaB }: { aId: number | null; bId: number | null; playerA: PlayerDetail | null; playerB: PlayerDetail | null; baseMetaA: number | null; baseMetaB: number | null }) {
  ...
  return <div className="hero-grid fc27-compare-page relative px-4 pb-24 pt-40 sm:px-6 sm:pt-48"><div className="hero-noise"/><div className="hero-orb hero-orb-primary"/><div className="relative mx-auto max-w-6xl"><p className="section-label">{c.eyebrow}</p><h1 className="mt-3 text-4xl font-bold sm:text-6xl">{c.title}</h1><p className="mt-4 max-w-2xl text-white/55">{c.subtitle}</p><Link href="/fc27/squad-builder" className="mt-3 inline-block text-sm font-semibold text-lime">{locale==="it"?"Deciso? Aggiungilo alla tua squadra →":"Decided? Add them to your squad →"}</Link>
