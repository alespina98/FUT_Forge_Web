...
export function DownloadPageContent({ release }: { release: ReleaseInfo }) {
  ...
  async function copyChecksum() { if (!release.sha256) return; await navigator.clipboard.writeText(release.sha256); setCopied(true); window.setTimeout(() => setCopied(false), 1800); }
  ...
  return <>
    <section className="download-hero hero-grid">
      ...
      <div className="download-hero-inner">
        ...
        <div className="download-chips"><span>v{release.version}</span><span>{d.windows}</span><span>{d.architecture}</span><span>{d.free}</span><span>{d.updates}</span></div>
      </div>
    </section>
    ...
    <section className="download-content section-shell" aria-labelledby="release-heading"><div className="release-card">
      ...
      {release.sha256 && <div className="checksum"><span>{d.checksum}</span><code>{release.sha256}</code><button type="button" onClick={copyChecksum} aria-label={d.copyChecksum}>{copied ? d.copied : d.copyChecksum}</button></div>}
      <a className="github-release-link" href={release.releaseUrl} target="_blank" rel="noreferrer">{d.officialRelease} ↗</a>
    </div></section>
    ...
    <section className="install-section section-shell"><div><p className="section-label">03</p><h2>{d.installation}</h2><ol>{d.installSteps.map((step, index) => <li key={step}><span>{index + 1}</span>{step}</li>)}</ol></div><div><p className="section-label">04</p><h2>{d.security}</h2><ul>{d.securityItems.map((item) => <li key={item}>{item}</li>)}</ul></div></section>
