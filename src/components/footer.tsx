  ...
  return (
    <footer data-reveal className="section-reveal border-t border-white/[.07] px-4 py-10 sm:px-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-8 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-x-7 gap-y-3 text-xs text-white/45"><Link href="/#features">{t.footer.features}</Link><Link href="/#roadmap">{t.footer.roadmap}</Link><Link href="/download">{t.footer.download}</Link><Link href="/privacy">{t.footer.privacy}</Link><a href="https://github.com/alespina98/FUT_Forge_Releases" target="_blank" rel="noopener noreferrer" aria-label={t.footer.contactAria}>{t.footer.contact}</a></div>
      </div>
