...
test("website bookmarklet gate uses the shared auth view and never legacy Supabase state",()=>{assert.match(source,/useWebsiteAuth\(\)/);assert.match(source,/authenticated \?/);assert.doesNotMatch(source,/useAuthUser|supabase|localStorage|sessionStorage/);});
