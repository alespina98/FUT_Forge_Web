...
export async function GET(request:NextRequest){
 const q=request.nextUrl.searchParams.get("q")?.trim()??"";if(q.length<2)return NextResponse.json({results:[]},{headers:NO_STORE});
 ...
 return NextResponse.json({results:rows},{headers:NO_STORE});
}
