...
// Already requires an Authorization header on the request (Cloudflare's own
// Workers Cache bypasses caching automatically for that), but explicit
// no-store here too so this route's policy never depends on that.
const NO_STORE = { "cache-control": "private, no-store, max-age=0" } as const;
export async function GET(request: Request) {
  const cronSecret = process.env.CRON_SECRET; const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!cronSecret || !authorized(request, cronSecret)) return NextResponse.json({ error: "Unauthorized" }, { status: 401, headers: NO_STORE });
  if (!serviceKey) return NextResponse.json({ error: "Ingestion is not configured" }, { status: 503, headers: NO_STORE });
  ...
  return NextResponse.json({ ok: results.every(result => !result.error), results }, { status: results.some(result => result.error) ? 207 : 200, headers: NO_STORE });
}
